...
// Generates a storage namespace key from the two person names
// so each couple has completely isolated data
const getUserPrefix = (p1, p2) => {
  if (!p1 || !p2) return "default";
  const clean = (s) => s.trim().toLowerCase().replace(/[^a-z0-9]/gi, "");
  return `${clean(p1)}_${clean(p2)}`;
};
...
export default function App() {
  ...
  const [showLavoriForm, setShowLavoriForm] = useState(false);
  const [editingLavoro, setEditingLavoro] = useState(null);
  const [lavoriFilterCat, setLavoriFilterCat] = useState("all");
  const [lavoriFilterStato, setLavoriFilterStato] = useState("all");
  const [collapsedLavoriCat, setCollapsedLavoriCat] = useState({});
  const [lavoriForm, setLavoriForm] = useState({
    fattura_num: "", data: today(), fornitore: "", categoria: "", prodotto: "",
    note: "", importo: "", iva_pct: "", tipo: "saldo",
    pagato_diego: "", pagato_carmen: "", pagato_insieme: "",
    da_pagare_diego: "", da_pagare_carmen: "", da_pagare_insieme: "",
    link_drive: "",
  });

  const key = `${year}-${String(month + 1).padStart(2, "0")}`;

  // Load user-specific data given a names-derived prefix
  const loadUserData = async (prefix) => {
    try { const h = await window.storage.get(`bgt-${prefix}-history`); if (h) setHistory(JSON.parse(h.value)); else setHistory({}); } catch { setHistory({}); }
    try { const p = await window.storage.get(`bgt-${prefix}-plan`); if (p) setPlan(JSON.parse(p.value)); else setPlan({}); } catch { setPlan({}); }
    try {
      const s = await window.storage.get(`bgt-${prefix}-structure`);
      if (s) {
        ...
        setStructure(merged);
      } else { setStructure(PLAN_STRUCTURE); }
    } catch { setStructure(PLAN_STRUCTURE); }
    try { const u = await window.storage.get(`bgt-${prefix}-uscite`); if (u) setUscite(JSON.parse(u.value)); else setUscite([]); } catch { setUscite([]); }
    try { const b = await window.storage.get(`bgt-${prefix}-bollette`); if (b) setBollette(JSON.parse(b.value)); else setBollette([]); } catch { setBollette([]); }
    try { const p = await window.storage.get(`bgt-${prefix}-pellet`); if (p) setPelletLog(JSON.parse(p.value)); else setPelletLog([]); } catch { setPelletLog([]); }
    try { const lv = await window.storage.get(`bgt-${prefix}-lavori`); if (lv) setLavori(JSON.parse(lv.value)); else setLavori([]); } catch { setLavori([]); }
  };

  useEffect(() => {
    async function load() {
      // 1. Load global settings (names + lang) — not user-specific
      let loadedNames = { p1: "", p2: "" };
      try { const n = await window.storage.get("bgt-names"); if (n) { loadedNames = JSON.parse(n.value); setNames(loadedNames); } } catch {}
      try { const l = await window.storage.get("bgt-lang"); if (l) setLang(l.value); } catch {}
      try { const p2 = await window.storage.get("bgt-pin"); if (p2) setPin(p2.value); } catch {}
      // 2. Load all user data under the user-specific prefix
      const prefix = getUserPrefix(loadedNames.p1, loadedNames.p2);
      await loadUserData(prefix);
      setLoaded(true);
    }
    load();
  }, []);

  const saveHistory = async (v) => { setHistory(v); await window.storage.set(`bgt-${getUserPrefix(names.p1,names.p2)}-history`, JSON.stringify(v)); };
  const saveNames = async (v) => {
    setNames(v);
    await window.storage.set("bgt-names", JSON.stringify(v));
    // Switch to the new user's data namespace
    const prefix = getUserPrefix(v.p1, v.p2);
    await loadUserData(prefix);
  };
  ...
  const deleteAccount = async () => {
    const pfx = getUserPrefix(names.p1, names.p2);
    for (const k of ["history","plan","structure","uscite","bollette","pellet","lavori"]) {
      try { await window.storage.delete(`bgt-${pfx}-${k}`); } catch {}
    }
    try { await window.storage.delete("bgt-names"); } catch {}
    try { await window.storage.delete("bgt-pin"); } catch {}
    setNames({ p1:"", p2:"" }); setPin(""); setHistory({}); setPlan({});
    setStructure(PLAN_STRUCTURE); setUscite([]); setBollette([]);
    setPelletLog([]); setLavori([]);
    setShowDeleteConfirm(false); setDeletePin(""); setDeletePinError(false); setShowSetup(false);
  };
  ...
  const exportData = () => {
    const data = { names, history, plan, uscite, bollette, pelletLog, lavori };
  ...
  const importData = async (jsonStr) => {
    try {
      ...
      // Names are saved globally; everything else under the names-derived prefix
      const importedNames = data.names || names;
      const pfx = getUserPrefix(importedNames.p1, importedNames.p2);
      if (data.names)     { setNames(data.names);         await window.storage.set("bgt-names",                    JSON.stringify(data.names)); }
      if (data.history)   { setHistory(data.history);     await window.storage.set(`bgt-${pfx}-history`,           JSON.stringify(data.history)); }
      if (data.plan)      { setPlan(data.plan);           await window.storage.set(`bgt-${pfx}-plan`,              JSON.stringify(data.plan)); }
      if (data.uscite)    { setUscite(data.uscite);       await window.storage.set(`bgt-${pfx}-uscite`,            JSON.stringify(data.uscite)); }
      if (data.bollette)  { setBollette(data.bollette);   await window.storage.set(`bgt-${pfx}-bollette`,          JSON.stringify(data.bollette)); }
      if (data.pelletLog) { setPelletLog(data.pelletLog); await window.storage.set(`bgt-${pfx}-pellet`,            JSON.stringify(data.pelletLog)); }
      if (data.lavori)    { setLavori(data.lavori);       await window.storage.set(`bgt-${pfx}-lavori`,            JSON.stringify(data.lavori)); }
      setImportStatus("ok");
  ...
  const savePlan = async (v) => { setPlan(v); await window.storage.set(`bgt-${getUserPrefix(names.p1,names.p2)}-plan`, JSON.stringify(v)); };
  const saveStructure = async (v) => { setStructure(v); await window.storage.set(`bgt-${getUserPrefix(names.p1,names.p2)}-structure`, JSON.stringify(v)); };
  const saveUscite = async (v) => { setUscite(v); await window.storage.set(`bgt-${getUserPrefix(names.p1,names.p2)}-uscite`, JSON.stringify(v)); };
  const saveBollette = async (v) => { setBollette(v); await window.storage.set(`bgt-${getUserPrefix(names.p1,names.p2)}-bollette`, JSON.stringify(v)); };
  const savePelletLog = async (v) => { setPelletLog(v); await window.storage.set(`bgt-${getUserPrefix(names.p1,names.p2)}-pellet`, JSON.stringify(v)); };
  const saveLavori = async (v) => { setLavori(v); await window.storage.set(`bgt-${getUserPrefix(names.p1,names.p2)}-lavori`, JSON.stringify(v)); };
  ...
  if (!hasNames || showSetup) return (
    <div style={{ minHeight:"100vh", background:"#0d0d0f", fontFamily:"'DM Sans','Segoe UI',sans-serif", color:"#f0ede8", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600&family=DM+Serif+Display&display=swap'); * { box-sizing:border-box; margin:0; padding:0; } input[type=text],input[type=password],select { background:#181820; border:1px solid #252530; color:#f0ede8; border-radius:10px; padding:14px 16px; font-size:15px; width:100%; outline:none; font-family:inherit; transition:border-color 0.2s; } input[type=text]:focus,input[type=password]:focus { border-color:#e8c547; } button { cursor:pointer; border:none; font-family:inherit; }`}</style>
      ...
      <div style={{ width:"100%", maxWidth:380, display:"flex", flexDirection:"column", gap:16 }}>
        ...
        <div>
          ...
          <input type="text" placeholder="es. Marco" value={setupDraft.p1 !== undefined ? setupDraft.p1 : names.p1} onChange={e => setSetupDraft(d => ({...d, p1: e.target.value}))} />
        </div>
        <div>
          <div style={{ fontSize:11, color:"#ff6b9d", fontWeight:700, letterSpacing:1, marginBottom:8 }}>{t("settingsPerson2").toUpperCase()}</div>
          <input type="text" placeholder="es. Sara" value={setupDraft.p2 !== undefined ? setupDraft.p2 : names.p2} onChange={e => setSetupDraft(d => ({...d, p2: e.target.value}))} />
        </div>

        {/* PIN field */}
        <div>
          <div style={{ fontSize:11, color:"#e74c3c", fontWeight:700, letterSpacing:1, marginBottom:4 }}>🔐 PIN DI SICUREZZA</div>
          <div style={{ fontSize:11, color:"#555", marginBottom:8 }}>
            {showSetup && pin ? "Lascia vuoto per non cambiarlo" : "Scegli un PIN per proteggere l'eliminazione dell'account"}
          </div>
          <input
            type="password"
            placeholder={showSetup && pin ? "••••••  (invariato)" : "Scegli un PIN (es. 1234)"}
            value={setupDraft.pin || ""}
            onChange={e => setSetupDraft(d => ({...d, pin: e.target.value}))}
            style={{ borderColor: !showSetup && !setupDraft.pin ? "rgba(231,76,60,0.4)" : undefined }}
          />
        </div>

        <button
          onClick={() => {
            const n = { p1: setupDraft.p1 !== undefined ? setupDraft.p1 : names.p1, p2: setupDraft.p2 !== undefined ? setupDraft.p2 : names.p2 };
            if (!n.p1 || !n.p2) return;
            if (!showSetup && !setupDraft.pin) return; // PIN obbligatorio al primo setup
            const newPin = setupDraft.pin || pin; // se non cambiato usa quello esistente
            saveNames(n, newPin);
            setSetupDraft({ p1:"", p2:"", pin:"" });
            setShowSetup(false);
          }}
          ...
          <>
            ...
            {!showDeleteConfirm ? (
              <button
                ...
                style={{ background:"rgba(231,76,60,0.1)", color:"#e74c3c", border:"1px solid rgba(231,76,60,0.3)", borderRadius:12, padding:"13px", fontSize:14, fontWeight:600 }}
              >
                🗑️ Elimina account e tutti i dati
              </button>
            ) : (
              <div style={{ background:"rgba(231,76,60,0.08)", border:"1px solid rgba(231,76,60,0.35)", borderRadius:12, padding:16 }}>
                <div style={{ fontSize:13, color:"#e74c3c", fontWeight:700, marginBottom:4 }}>⚠️ Eliminazione account</div>
                <div style={{ fontSize:12, color:"#888", marginBottom:12 }}>Tutti i dati verranno cancellati definitivamente. Inserisci il tuo PIN per confermare.</div>
                <input
                  type="password"
                ...
                />
                {deletePinError && <div style={{ fontSize:12, color:"#e74c3c", marginBottom:8 }}>❌ PIN errato</div>}
                <div style={{ display:"flex", gap:8 }}>
                  <button
                    ...
                    style={{ flex:1, background:"#e74c3c", color:"#fff", borderRadius:10, padding:"11px", fontSize:13, fontWeight:700 }}
                  >
                    Conferma eliminazione
                  </button>
                  <button
                    onClick={() => { setShowDeleteConfirm(false); setDeletePin(""); setDeletePinError(false); }}
                    style={{ flex:1, background:"#1f1f28", color:"#888", borderRadius:10, padding:"11px", fontSize:13 }}
                  >
                    Annulla
                  </button>
                </div>
            ...
            <button onClick={() => { setShowSetup(false); setSetupDraft({p1:"",p2:"",pin:""}); setShowDeleteConfirm(false); setDeletePin(""); }} style={{ background:"transparent", color:"#555", padding:"10px", fontSize:13 }}>
              {t("settingsCancel")}
            </button>
          </>
        )}
    ...
    <div style={{ minHeight:"100vh", background:"#0d0d0f", fontFamily:"'DM Sans','Segoe UI',sans-serif", color:"#f0ede8" }}>
      ...
      <div style={{ padding:"16px 16px 0", borderBottom:"1px solid #1a1a22", position:"sticky", top:0, background:"#0d0d0f", zIndex:20 }}>
        <div style={{ maxWidth:680, margin:"0 auto" }}>
          ...
          {/* Main tabs */}
          <div style={{ display:"flex", borderBottom:"1px solid #1a1a22", overflowX:"auto" }}>
            {[["entrate","💰","tabEntrate"],["budget","📋","tabBudget"],["spese","💸","tabSpese"],["casa","🏠","tabCasa"],["lavori","🔨","Lavori"]].map(([t_,icon,tkey]) => (
              <button key={t_} onClick={() => setTab(t_)} style={{ flex:1, minWidth:52, padding:"9px 0", fontSize:11, fontWeight:tab===t_?700:400, color:tab===t_?"#e8c547":"#555", borderBottom:tab===t_?"2px solid #e8c547":"2px solid transparent", transition:"all 0.2s", whiteSpace:"nowrap" }}>
                {icon}<br/><span style={{ fontSize:10 }}>{tkey === "Lavori" ? "Lavori" : t(tkey)}</span>
              </button>
            ))}
          </div>
          {/* Sub-tabs */}
      ...
      <div style={{ maxWidth:680, margin:"0 auto", padding:"20px 16px 70px" }}>
        ...
        {(tab === "casa" && subTab === "pellet") && (() => {
          ...
          const todayDate2 = new Date(); todayDate2.setHours(0,0,0,0);
          let rows = sorted.map((entry, i) => {
            rimasti += (parseInt(entry.comprati) || 0) - (parseInt(entry.consumati) || 0);
            // Media: total consumed / actual days elapsed from first entry date
            const totalConsumed = sorted.slice(0, i+1).reduce((a, e) => a + (parseInt(e.consumati)||0), 0);
            const isLast = i === sorted.length - 1;
            const refDate = isLast ? todayDate2 : new Date(entry.date);
            const daysElapsed = Math.max(1, Math.round((refDate - firstDate) / (1000 * 60 * 60 * 24)) + 1);
        ...
        {tab === "lavori" && (() => {
          const TIPO_COLORS = {
            preventivo: "#8e44ad", acconto: "#2980b9", saldo: "#27ae60",
            totale: "#e67e22", contanti: "#7f8c8d", extra: "#e74c3c"
          };
          const TIPO_LABELS = {
            preventivo: "Preventivo", acconto: "Acconto", saldo: "Saldo Finale",
            totale: "Totale", contanti: "Contanti", extra: "Extra"
          };

          const n = (v) => parseFloat(v) || 0;
          const lavTotale = lavori.reduce((a, l) => a + n(l.importo), 0);
          const lavPagato = lavori.reduce((a, l) => a + n(l.pagato_diego) + n(l.pagato_carmen) + n(l.pagato_insieme), 0);
          const lavDaPagare = lavori.reduce((a, l) => a + n(l.da_pagare_diego) + n(l.da_pagare_carmen) + n(l.da_pagare_insieme), 0);
          const lavPagatoDiego = lavori.reduce((a, l) => a + n(l.pagato_diego) + n(l.pagato_insieme) * (diegoPct / 100), 0);
          const lavPagatoCarmen = lavori.reduce((a, l) => a + n(l.pagato_carmen) + n(l.pagato_insieme) * (carmenPct / 100), 0);

          const catList = [...new Set(lavori.map(l => l.categoria).filter(Boolean))].sort();

          const filtered = lavori.filter(l => {
            if (lavoriFilterCat !== "all" && l.categoria !== lavoriFilterCat) return false;
            if (lavoriFilterStato === "pagato") return n(l.pagato_diego) + n(l.pagato_carmen) + n(l.pagato_insieme) > 0 && n(l.da_pagare_diego) + n(l.da_pagare_carmen) + n(l.da_pagare_insieme) === 0;
            if (lavoriFilterStato === "da_pagare") return n(l.da_pagare_diego) + n(l.da_pagare_carmen) + n(l.da_pagare_insieme) > 0;
            return true;
          }).sort((a, b) => {
          ...
          const grouped = {};
          filtered.forEach(l => {
            const cat = l.categoria || "— Senza categoria";
            if (!grouped[cat]) grouped[cat] = [];
            grouped[cat].push(l);
          });

          const resetForm = () => setLavoriForm({
            fattura_num:"", data:today(), fornitore:"", categoria:"", prodotto:"",
            note:"", importo:"", iva_pct:"", tipo:"saldo",
            pagato_diego:"", pagato_carmen:"", pagato_insieme:"",
            da_pagare_diego:"", da_pagare_carmen:"", da_pagare_insieme:"",
          ...
          const saveLavoro = () => {
            if (!lavoriForm.prodotto && !lavoriForm.fornitore) return;
            const entry = { ...lavoriForm, id: editingLavoro || Date.now() };
            if (editingLavoro) {
              saveLavori(lavori.map(l => l.id === editingLavoro ? entry : l));
            } else {
              saveLavori([entry, ...lavori]);
            }
            setShowLavoriForm(false);
            setEditingLavoro(null);
            resetForm();
          };

          const startEditLavoro = (l) => {
            setEditingLavoro(l.id);
            setLavoriForm({ ...l });
            setShowLavoriForm(true);
            window.scrollTo({ top: 0, behavior: "smooth" });
          };

          const deleteLavoro = (id) => saveLavori(lavori.filter(l => l.id !== id));
          ...
          const getStatoLavoro = (l) => {
            const pag = n(l.pagato_diego) + n(l.pagato_carmen) + n(l.pagato_insieme);
            const dp = n(l.da_pagare_diego) + n(l.da_pagare_carmen) + n(l.da_pagare_insieme);
            if (dp > 0 && pag > 0) return { label: "Parz. Pagato", color: "#e67e22", bg: "rgba(230,126,34,0.15)" };
            if (dp > 0) return { label: "Da Pagare", color: "#e74c3c", bg: "rgba(231,76,60,0.15)" };
            if (pag > 0) return { label: "Pagato", color: "#27ae60", bg: "rgba(39,174,96,0.15)" };
            return { label: "Preventivo", color: "#8e44ad", bg: "rgba(142,68,173,0.15)" };
          };
          ...
          return (
            <div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:16 }}>
                <div style={{ background:"#141418", border:"1px solid #222", borderRadius:12, padding:"12px 14px" }}>
                  <div style={{ fontSize:9, color:"#555", letterSpacing:1, textTransform:"uppercase", marginBottom:5 }}>Totale Lavori</div>
                  <div style={{ fontFamily:"'DM Serif Display',serif", fontSize:16, color:"#e8c547" }}>{fmt(lavTotale)}</div>
                  <div style={{ fontSize:10, color:"#444", marginTop:2 }}>{lavori.length} voci</div>
                </div>
                <div style={{ background:"rgba(39,174,96,0.07)", border:"1px solid rgba(39,174,96,0.2)", borderRadius:12, padding:"12px 14px" }}>
                  <div style={{ fontSize:9, color:"#555", letterSpacing:1, textTransform:"uppercase", marginBottom:5 }}>✅ Pagato</div>
                  <div style={{ fontFamily:"'DM Serif Display',serif", fontSize:16, color:"#27ae60" }}>{fmt(lavPagato)}</div>
                  <div style={{ fontSize:10, color:"#444", marginTop:2 }}>{lavPagato > 0 ? ((lavPagato/Math.max(lavTotale,1))*100).toFixed(0) : 0}%</div>
                </div>
                <div style={{ background:"rgba(231,76,60,0.07)", border:"1px solid rgba(231,76,60,0.2)", borderRadius:12, padding:"12px 14px" }}>
                  <div style={{ fontSize:9, color:"#555", letterSpacing:1, textTransform:"uppercase", marginBottom:5 }}>⏳ Da Pagare</div>
                  <div style={{ fontFamily:"'DM Serif Display',serif", fontSize:16, color:"#e74c3c" }}>{fmt(lavDaPagare)}</div>
                  <div style={{ fontSize:10, color:"#444", marginTop:2 }}>{lavDaPagare > 0 ? ((lavDaPagare/Math.max(lavTotale,1))*100).toFixed(0) : 0}%</div>
                </div>
              </div>

              {/* Per-person summary */}
              {lavPagato > 0 && (
                <div style={{ background:"#131317", border:"1px solid #1f1f28", borderRadius:12, padding:"12px 14px", marginBottom:16 }}>
                  <div style={{ fontSize:9, color:"#444", letterSpacing:1, textTransform:"uppercase", marginBottom:10 }}>Ripartizione Pagato</div>
                  <div style={{ display:"flex", gap:8 }}>
                    {[[N1, lavPagatoDiego, "#4a9eff"], [N2, lavPagatoCarmen, "#ff6b9d"]].map(([lbl, val, col]) => (
                      <div key={lbl} style={{ flex:1, background:`${col}0d`, border:`1px solid ${col}22`, borderRadius:9, padding:"10px 12px" }}>
                        <div style={{ fontSize:9, color:col, fontWeight:700, letterSpacing:1, marginBottom:3 }}>{lbl}</div>
                        <div style={{ fontSize:15, fontFamily:"'DM Serif Display',serif" }}>{fmt(val)}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Add button */}
              <button onClick={() => { resetForm(); setEditingLavoro(null); setShowLavoriForm(!showLavoriForm); }} style={{ width:"100%", background: showLavoriForm ? "#1f1f28" : "#e8c547", color: showLavoriForm ? "#555" : "#0d0d0f", borderRadius:12, padding:"12px", fontSize:14, fontWeight:600, marginBottom:14, transition:"all 0.2s" }}>
                {showLavoriForm && !editingLavoro ? "✕ Annulla" : editingLavoro ? "✏️ Modifica Voce" : "🔨 + Nuova Voce"}
              </button>
              ...
              {showLavoriForm && (
                <div style={{ background:"#141418", border:"1px solid #252530", borderRadius:14, padding:16, marginBottom:16 }}>
                  <div style={{ fontSize:11, color:"#555", letterSpacing:1, textTransform:"uppercase", marginBottom:14 }}>
                    {editingLavoro ? "✏️ Modifica Voce" : "➕ Nuova Voce"}
                  </div>
                  <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                      <div>
                        <div style={{ fontSize:10, color:"#666", marginBottom:4 }}>N° Fattura</div>
                        <input type="text" placeholder="es. 137, FPR 23/25..." value={lavoriForm.fattura_num} onChange={e => setLavoriForm(f=>({...f, fattura_num:e.target.value}))} />
                      </div>
                      <div>
                        <div style={{ fontSize:10, color:"#666", marginBottom:4 }}>Data Fattura</div>
                        <input type="date" value={lavoriForm.data} onChange={e => setLavoriForm(f=>({...f, data:e.target.value}))} />
                      </div>
                    </div>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                      <div>
                        <div style={{ fontSize:10, color:"#666", marginBottom:4 }}>Fornitore</div>
                        <input type="text" placeholder="es. Eliano, Aurelio..." value={lavoriForm.fornitore} onChange={e => setLavoriForm(f=>({...f, fornitore:e.target.value}))} />
                      </div>
                      <div>
                        <div style={{ fontSize:10, color:"#666", marginBottom:4 }}>Categoria / Progetto</div>
                        <input type="text" placeholder="es. Caldaia Pellet..." value={lavoriForm.categoria} onChange={e => setLavoriForm(f=>({...f, categoria:e.target.value}))} list="cat-suggestions" />
                        <datalist id="cat-suggestions">{catList.map(c => <option key={c} value={c} />)}</datalist>
                      </div>
                    </div>

                    {/* Prodotto */}
                    <div>
                      <div style={{ fontSize:10, color:"#666", marginBottom:4 }}>Descrizione Lavoro</div>
                      <input type="text" placeholder="es. Installazione caldaia a pellet..." value={lavoriForm.prodotto} onChange={e => setLavoriForm(f=>({...f, prodotto:e.target.value}))} />
                    </div>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
                      <div>
                        ...
                        <select value={lavoriForm.tipo} onChange={e => setLavoriForm(f=>({...f, tipo:e.target.value}))}>
                          {Object.entries(TIPO_LABELS).map(([k,v]) => <option key={k} value={k}>{v}</option>)}
                        </select>
                      </div>
                      <div>
                        <div style={{ fontSize:10, color:"#666", marginBottom:4 }}>Importo (€)</div>
                        <div style={{ position:"relative" }}>
                          <span style={{ position:"absolute", left:10, top:"50%", transform:"translateY(-50%)", color:"#555", fontSize:12 }}>€</span>
                          <input type="number" placeholder="0,00" value={lavoriForm.importo} onChange={e => setLavoriForm(f=>({...f, importo:e.target.value}))} style={{ paddingLeft:24 }} />
                        </div>
                      </div>
                      <div>
                        <div style={{ fontSize:10, color:"#666", marginBottom:4 }}>IVA %</div>
                        <input type="number" placeholder="es. 10, 22" value={lavoriForm.iva_pct} onChange={e => setLavoriForm(f=>({...f, iva_pct:e.target.value}))} />
                      </div>
                    </div>

                    {/* Divider Pagato */}
                    <div style={{ borderTop:"1px dashed #252530", paddingTop:10 }}>
                      <div style={{ fontSize:10, color:"#27ae60", fontWeight:700, letterSpacing:1, textTransform:"uppercase", marginBottom:8 }}>✅ Già Pagato</div>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
                        {[["pagato_diego", N1, "#4a9eff"],["pagato_carmen", N2, "#ff6b9d"],["pagato_insieme", "Insieme", "#e8c547"]].map(([field, lbl, col]) => (
                          <div key={field}>
                            <div style={{ fontSize:9, color:col, fontWeight:700, marginBottom:3 }}>{lbl}</div>
                            <div style={{ position:"relative" }}>
                              <span style={{ position:"absolute", left:8, top:"50%", transform:"translateY(-50%)", color:"#555", fontSize:11 }}>€</span>
                              <input type="number" placeholder="0" value={lavoriForm[field]} onChange={e => setLavoriForm(f=>({...f,[field]:e.target.value}))} style={{ paddingLeft:20, padding:"8px 8px 8px 20px", fontSize:12 }} />
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    {/* Divider Da Pagare */}
                    <div style={{ borderTop:"1px dashed #252530", paddingTop:10 }}>
                      <div style={{ fontSize:10, color:"#e74c3c", fontWeight:700, letterSpacing:1, textTransform:"uppercase", marginBottom:8 }}>⏳ Da Pagare</div>
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
                        {[["da_pagare_diego", N1, "#4a9eff"],["da_pagare_carmen", N2, "#ff6b9d"],["da_pagare_insieme", "Insieme", "#e8c547"]].map(([field, lbl, col]) => (
                          <div key={field}>
                            <div style={{ fontSize:9, color:col, fontWeight:700, marginBottom:3 }}>{lbl}</div>
                            <div style={{ position:"relative" }}>
                              <span style={{ position:"absolute", left:8, top:"50%", transform:"translateY(-50%)", color:"#555", fontSize:11 }}>€</span>
                    ...
                    <div>
                      ...
                      <input type="text" placeholder="Note aggiuntive..." value={lavoriForm.note} onChange={e => setLavoriForm(f=>({...f, note:e.target.value}))} />
                    </div>
                    <div>
                      <div style={{ fontSize:10, color:"#4a9eff", marginBottom:4 }}>🔗 Link Drive (fattura)</div>
                      <input type="text" placeholder="https://drive.google.com/..." value={lavoriForm.link_drive} onChange={e => setLavoriForm(f=>({...f, link_drive:e.target.value}))} />
                    </div>

                    <button onClick={saveLavoro} style={{ background:"#e8c547", color:"#0d0d0f", borderRadius:10, padding:"12px", fontSize:14, fontWeight:700, marginTop:2 }}>
                      {editingLavoro ? "💾 Salva Modifiche" : "➕ Aggiungi Voce"}
                    </button>
                  </div>
                </div>
              )}
              ...
              {lavori.length > 0 && (
                <div style={{ display:"flex", gap:8, marginBottom:14 }}>
                  <select value={lavoriFilterCat} onChange={e => setLavoriFilterCat(e.target.value)} style={{ flex:2, padding:"7px 10px", fontSize:12 }}>
                    <option value="all">Tutti i Progetti</option>
                    {catList.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                  <select value={lavoriFilterStato} onChange={e => setLavoriFilterStato(e.target.value)} style={{ flex:1, padding:"7px 10px", fontSize:12 }}>
              ...
              {lavori.length === 0 ? (
                <div style={{ textAlign:"center", color:"#333", fontSize:13, padding:"40px 0" }}>
                  ...
                  Nessun lavoro ancora. Usa il bottone qui sopra o importa i dati.
                </div>
              ) : (
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  {Object.entries(grouped).map(([cat, items]) => {
                    const catTotal = items.reduce((a, l) => a + n(l.importo), 0);
                    const catPagato = items.reduce((a, l) => a + n(l.pagato_diego) + n(l.pagato_carmen) + n(l.pagato_insieme), 0);
                    const catDaPagare = items.reduce((a, l) => a + n(l.da_pagare_diego) + n(l.da_pagare_carmen) + n(l.da_pagare_insieme), 0);
                    const isCollapsed = collapsedLavoriCat[cat];
                    return (
                      <div key={cat} style={{ border:"1px solid #1f1f28", borderRadius:12, overflow:"hidden" }}>
                        {/* Category header */}
                        <button onClick={() => setCollapsedLavoriCat(c=>({...c,[cat]:!c[cat]}))} style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", background:"#141418", padding:"11px 14px" }}>
                          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                            <div style={{ width:3, height:18, borderRadius:2, background:"#e8c547", flexShrink:0 }} />
                            <span style={{ fontSize:13, color:"#ddd", fontWeight:600 }}>{cat}</span>
                            <span style={{ fontSize:11, color:"#555" }}>{items.length} voci</span>
                          </div>
                          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                            {catDaPagare > 0 && <span style={{ fontSize:11, color:"#e74c3c", background:"rgba(231,76,60,0.1)", border:"1px solid rgba(231,76,60,0.3)", borderRadius:6, padding:"2px 7px" }}>⏳ {fmt(catDaPagare)}</span>}
                            {catPagato > 0 && <span style={{ fontSize:11, color:"#27ae60", background:"rgba(39,174,96,0.1)", border:"1px solid rgba(39,174,96,0.3)", borderRadius:6, padding:"2px 7px" }}>✅ {fmt(catPagato)}</span>}
                            <span style={{ color:"#444", fontSize:11 }}>{isCollapsed ? "▶" : "▼"}</span>
                          </div>
                        </button>
                        {!isCollapsed && (
                          <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
                            {items.map((l, idx) => {
                              const stato = getStatoLavoro(l);
                              const tipoColor = TIPO_COLORS[l.tipo] || "#7f8c8d";
                              const tipoLabel = TIPO_LABELS[l.tipo] || l.tipo;
                              const pag = n(l.pagato_diego) + n(l.pagato_carmen) + n(l.pagato_insieme);
                              const dp = n(l.da_pagare_diego) + n(l.da_pagare_carmen) + n(l.da_pagare_insieme);
                              return (
                                <div key={l.id} style={{ background: idx % 2 === 0 ? "#0f0f12" : "#111115", borderTop:"1px solid #1a1a22", padding:"12px 14px" }}>
                                  {/* Header row */}
                                  <div style={{ display:"flex", alignItems:"flex-start", gap:8, marginBottom:6 }}>
                                    <div style={{ flex:1, minWidth:0 }}>
                                      <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", gap:5, marginBottom:3 }}>
                                        <span className="tag" style={{ background:tipoColor, fontSize:9 }}>{tipoLabel}</span>
                                        <span style={{ fontSize:10, color:"#555" }}>{formatDateShort(l.data)}</span>
                                        {l.fattura_num && l.fattura_num !== "—" && (
                                          <span style={{ fontSize:10, color:"#444", background:"#1f1f28", borderRadius:4, padding:"1px 6px" }}>#{l.fattura_num}</span>
                                        )}
                                        <span style={{ fontSize:10, fontWeight:600, padding:"1px 7px", borderRadius:10, background:stato.bg, color:stato.color }}>{stato.label}</span>
                                      </div>
                                      <div style={{ fontSize:14, color:"#ddd", fontWeight:500, marginBottom:l.fornitore ? 2 : 0 }}>{l.prodotto || "—"}</div>
                                      {l.fornitore && <div style={{ fontSize:11, color:"#777" }}>👤 {l.fornitore}</div>}
                                      {l.note && <div style={{ fontSize:11, color:"#555", fontStyle:"italic", marginTop:2 }}>📝 {l.note}</div>}
                                    </div>
                                    <div style={{ textAlign:"right", flexShrink:0 }}>
                                      <div style={{ fontFamily:"'DM Serif Display',serif", fontSize:16, color:"#f0ede8" }}>{fmt(n(l.importo))}</div>
                                      {l.iva_pct && <div style={{ fontSize:10, color:"#555" }}>IVA {l.iva_pct}%</div>}
                                    </div>
                                  </div>
                                    ...
                                    <div style={{ display:"flex", gap:5, marginBottom:6, flexWrap:"wrap" }}>
                                      {pag > 0 && (
                                        <div style={{ display:"flex", gap:4 }}>
                                          {n(l.pagato_diego) > 0 && <span style={{ fontSize:10, color:"#4a9eff", background:"rgba(74,158,255,0.1)", border:"1px solid rgba(74,158,255,0.2)", borderRadius:5, padding:"2px 7px" }}>✅ {N1} {fmt(n(l.pagato_diego))}</span>}
                                          {n(l.pagato_carmen) > 0 && <span style={{ fontSize:10, color:"#ff6b9d", background:"rgba(255,107,157,0.1)", border:"1px solid rgba(255,107,157,0.2)", borderRadius:5, padding:"2px 7px" }}>✅ {N2} {fmt(n(l.pagato_carmen))}</span>}
                                          {n(l.pagato_insieme) > 0 && <span style={{ fontSize:10, color:"#27ae60", background:"rgba(39,174,96,0.1)", border:"1px solid rgba(39,174,96,0.2)", borderRadius:5, padding:"2px 7px" }}>✅ Insieme {fmt(n(l.pagato_insieme))}</span>}
                                        </div>
                                      )}
                                      {dp > 0 && (
                                        <div style={{ display:"flex", gap:4 }}>
                                          {n(l.da_pagare_diego) > 0 && <span style={{ fontSize:10, color:"#e74c3c", background:"rgba(231,76,60,0.1)", border:"1px solid rgba(231,76,60,0.2)", borderRadius:5, padding:"2px 7px" }}>⏳ {N1} {fmt(n(l.da_pagare_diego))}</span>}
                                          {n(l.da_pagare_carmen) > 0 && <span style={{ fontSize:10, color:"#e74c3c", background:"rgba(231,76,60,0.1)", border:"1px solid rgba(231,76,60,0.2)", borderRadius:5, padding:"2px 7px" }}>⏳ {N2} {fmt(n(l.da_pagare_carmen))}</span>}
                                          {n(l.da_pagare_insieme) > 0 && <span style={{ fontSize:10, color:"#e74c3c", background:"rgba(231,76,60,0.1)", border:"1px solid rgba(231,76,60,0.2)", borderRadius:5, padding:"2px 7px" }}>⏳ Insieme {fmt(n(l.da_pagare_insieme))}</span>}
                                        </div>
                                      )}
                                    </div>
                                  )}
                                  ...
                                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                                    <div>
                                      {l.link_drive && (
                                        <a href={l.link_drive} target="_blank" rel="noopener noreferrer" style={{ fontSize:11, color:"#4a9eff", textDecoration:"none", display:"inline-flex", alignItems:"center", gap:4, background:"rgba(74,158,255,0.1)", border:"1px solid rgba(74,158,255,0.25)", borderRadius:6, padding:"3px 9px" }}>
                                          📁 Apri Fattura
                                        </a>
                                      )}
                                    </div>
                                    <div style={{ display:"flex", gap:6 }}>
                                      <button onClick={() => startEditLavoro(l)} style={{ fontSize:11, color:"#555", padding:"4px 9px", border:"1px solid #2a2a35", borderRadius:6 }}>✏️</button>
                                      <button onClick={() => deleteLavoro(l.id)} style={{ fontSize:11, color:"#c0392b", padding:"4px 9px", border:"1px solid rgba(192,57,43,0.3)", borderRadius:6 }}>🗑</button>
                                    </div>
                                  </div>
                                </div>
                              );
                            ...
                            {/* Category footer */}
                            <div style={{ background:"#0d0d10", padding:"8px 14px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                              <span style={{ fontSize:10, color:"#444" }}>Totale {cat}</span>
                              <span style={{ fontSize:13, fontFamily:"'DM Serif Display',serif", color:"#e8c547" }}>{fmt(catTotal)}</span>
                            </div>
                          </div>
                        )}
                      </div>
              ...
              {/* Grand total footer */}
              {lavori.length > 0 && (
                <div style={{ marginTop:20, background:"#131317", border:"1px solid #1f1f28", borderRadius:14, padding:"14px 16px" }}>
                  <div style={{ fontSize:10, color:"#444", letterSpacing:1.5, textTransform:"uppercase", marginBottom:12 }}>Riepilogo Totale Lavori</div>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
                    <span style={{ color:"#666", fontSize:13 }}>Totale commissionato</span>
                    <span style={{ fontFamily:"'DM Serif Display',serif", fontSize:22, color:"#e8c547" }}>{fmt(lavTotale)}</span>
                  </div>
                  <div style={{ height:8, borderRadius:4, overflow:"hidden", display:"flex", marginBottom:10 }}>
                    <div style={{ width:`${lavTotale > 0 ? (lavPagato/lavTotale)*100 : 0}%`, background:"#27ae60", transition:"width 0.5s" }} />
                    <div style={{ width:`${lavTotale > 0 ? (lavDaPagare/lavTotale)*100 : 0}%`, background:"#e74c3c", transition:"width 0.5s" }} />
                  </div>
                  <div style={{ display:"flex", justifyContent:"space-between" }}>
                    <span style={{ fontSize:12, color:"#27ae60" }}>✅ Pagato {fmt(lavPagato)}</span>
                    <span style={{ fontSize:12, color:"#e74c3c" }}>⏳ Da Pagare {fmt(lavDaPagare)}</span>
                  </div>
                </div>
              )}
            </div>
